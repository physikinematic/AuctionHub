...
const getAll = async (query) => {
  ...
  return response("Batch Auctions Retrieved Successfully", auctions, {
    pagination: {
      total: total,
      page: _page,
      limit: _limit,
...
const getOwned = async (params, query) => {
  ...
  const account = await Account.findOne({ _id: ownerId });
  if (!account) {
    throw new RequestError(204, `Account ${ownerId} Not Found`);
  }

  const _page = parseInt(page);
  const _limit = parseInt(limit);

  const auctions = await Auction.find({ ownerId: ownerId })
    .skip((_page - 1) * _limit)
    .limit(_limit);

  const total = await Auction.countDocuments({ ownerId: ownerId });

  return response("Owned Auctions Retrieved Successfully", auctions, {
    pagination: {
      total: total,
      page: _page,
      limit: _limit,
...
const getBidded = async (params, query) => {
  ...
  const auctions = await Auction.find({
    bids: { $elemMatch: { ownerId: ownerId } },
  })
    .skip((_page - 1) * _limit)
    .limit(_limit);
